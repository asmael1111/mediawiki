...
class PersonalTools extends Component {
	...
	protected function getNewtalkNotifier() {
		...
		$newMessagesAlert = $this->getSkinTemplate()->getMsg( 'changeling-newmessages' )->text();
	...
	protected function getDropdownToggle(): string {
		...
		if ( $user->isLoggedIn() ) {
			...
			$toolsLinkText = $this->getSkinTemplate()->getMsg( 'changeling-loggedin' )->
				params( $user->getName() )->text();

		} else {

			$toolsClass = 'navbar-usernotloggedin';
			$toolsLinkText = $this->getSkinTemplate()->getMsg( 'changeling-notloggedin' )->text();

		}

		Hooks::run( 'ChangelingNavbarHorizontalPersonalToolsLinkText', [ &$toolsLinkText,
			$this->getSkin() ] );
