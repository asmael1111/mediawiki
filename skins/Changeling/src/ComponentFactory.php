...
use Skins\Changeling\Components\Container;
...
class ComponentFactory {
	...
	private const NAMESPACE_HIERARCHY = 'Skins\\Changeling\\Components';
	...
	protected function getModifiedComponent( DOMElement $description, Component $component ) {
		...
		$className = 'Skins\\Changeling\\Components\\Modifications\\' .
			$description->getAttribute( 'type' );

		if ( !class_exists( $className ) || !is_subclass_of( $className,
			'Skins\\Changeling\\Components\\Modifications\\Modification' ) ) {
			throw new MWException(
	...
	protected function mapComponentDescriptionToClassName( DOMElement $description ) {
		if ( $description->hasAttribute( 'type' ) ) {
			...
			$changelingClassName = implode( '\\', [ self::NAMESPACE_HIERARCHY, $className ] );
			if ( class_exists( $changelingClassName ) ) {
				return $changelingClassName;
			}
