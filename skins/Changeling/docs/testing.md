...
From the Changeling installation directory run:
```sh
...
`printerClass="Skins\Changeling\Tests\Util\ColoringTextUIResultPrinter"` as an
additional attribute in the `phpunit` element to colorize then tests that
connect to the external service.

[travis]: https://travis-ci.org/ProfessionalWiki/changeling
...
Testing of the Changeling styling is done in a two-dimensional test
...
fine on a x-large screen. The standard collapse-point at which Changeling switches
