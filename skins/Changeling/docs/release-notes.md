...
* Added theme support via the new `ChangelingThemeFile` setting (thanks @malberts)
* Added grid breakpoint override support via the new `$cmln-grid-breakpoints` SCSS variable (thanks @malberts)
* Improved MultimediaViewer extension support (requires [cache update](https://github.com/malberts/changeling/blob/issue-178/docs/customization.md#triggering-a-cache-update)) (thanks @malberts) 
* Added external link icons support via the new `ChangelingEnableExternalLinkIcons` setting (thanks @malberts)
...
* Added `ChangelingSkinTemplateOutputPageBeforeExec` hook (thanks @jdlrobson)
...
Fixes:
* ([#87](https://github.com/ProfessionalWiki/changeling/issues/87)) Show formedit button on the navbar
* ([#89](https://github.com/ProfessionalWiki/changeling/issues/89)) Show default icon for unknown page tools on the navbar
* ([#91](https://github.com/ProfessionalWiki/changeling/issues/91)) Ensure mediawiki.skinning.content module is loadable
...
* Rename `$cgChangelingExternalLessVariables` to `$cgChangelingExternalStyleVariables`
...
Changes:
* (#49) Allows skins that build on top of Changeling to set a specific layout
...
* Add ChangelingNavbarHorizontalPersonalToolsLinkText hook
* Add ChangelingNavbarHorizontalNewTalkLinkText hook
...
of the Changeling skin for MediaWiki.
