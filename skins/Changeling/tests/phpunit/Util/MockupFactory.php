...
namespace Skins\Changeling\Tests\Util;
...
use Skins\Changeling\ComponentFactory;
...
class MockupFactory {
	...
	/**
	 * @return \PHPUnit\Framework\MockObject\MockObject|ChangelingTemplate
	 * @throws \MWException
	 */
	public function getChangelingSkinTemplateStub() {
		$changelingTemplate = $this->testCase->getMockBuilder( ChangelingTemplate::class )
			->disableOriginalConstructor()
			->getMock();

		$changelingTemplate->data = $this->getSkinTemplateDummyDataSetForMainNamespace();
		$changelingTemplate->translator = $this->getTranslatorStub();

		$dataMap = array_map(
			function ( $key, $value ) {
				return [ $key, null, $value ];
			},
			array_keys( $changelingTemplate->data ),
			array_values( $changelingTemplate->data )
		);

		$changelingTemplate->expects( $this->testCase->any() )
			->method( 'get' )
	...
	protected function getSkinStub() {
		...
		$skin = $this->testCase->getMockBuilder( Changeling::class )
			->disableOriginalConstructor()
			->getMock();
