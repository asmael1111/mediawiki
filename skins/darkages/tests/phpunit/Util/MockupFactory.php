...
namespace Skins\Darkages\Tests\Util;
...
use Skins\Darkages\ComponentFactory;
...
class MockupFactory {
	...
	/**
	 * @return \PHPUnit\Framework\MockObject\MockObject|DarkagesTemplate
	 * @throws \MWException
	 */
	public function getDarkagesSkinTemplateStub() {
		$darkagesTemplate = $this->testCase->getMockBuilder( DarkagesTemplate::class )
			->disableOriginalConstructor()
			->getMock();

		$darkagesTemplate->data = $this->getSkinTemplateDummyDataSetForMainNamespace();
		$darkagesTemplate->translator = $this->getTranslatorStub();

		$dataMap = array_map(
			function ( $key, $value ) {
				return [ $key, null, $value ];
			},
			array_keys( $darkagesTemplate->data ),
			array_values( $darkagesTemplate->data )
		);

		$darkagesTemplate->expects( $this->testCase->any() )
			->method( 'get' )
	...
	protected function getSkinStub() {
		...
		$skin = $this->testCase->getMockBuilder( Darkages::class )
			->disableOriginalConstructor()
			->getMock();
