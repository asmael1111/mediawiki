...
namespace Skins\Darkages\Components\NavbarHorizontal;

use Hooks;
use Skins\Darkages\Components\Component;
use Skins\Darkages\IdRegistry;
...
class PersonalTools extends Component {
	...
	protected function getNewtalkNotifier() {
		...
		$newMessagesAlert = $this->getSkinTemplate()->getMsg( 'darkages-newmessages' )->text();
	...
	protected function getDropdownToggle(): string {
		...
		if ( $user->isLoggedIn() ) {
			...
			$toolsLinkText = $this->getSkinTemplate()->getMsg( 'darkages-loggedin' )->
				params( $user->getName() )->text();

		} else {

			$toolsClass = 'navbar-usernotloggedin';
			$toolsLinkText = $this->getSkinTemplate()->getMsg( 'darkages-notloggedin' )->text();

		}

		Hooks::run( 'DarkagesNavbarHorizontalPersonalToolsLinkText', [ &$toolsLinkText,
			$this->getSkin() ] );
