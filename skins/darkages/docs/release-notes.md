...
* Added theme support via the new `DarkagesThemeFile` setting (thanks @malberts)
* Added grid breakpoint override support via the new `$cmln-grid-breakpoints` SCSS variable (thanks @malberts)
* Improved MultimediaViewer extension support (requires [cache update](https://github.com/malberts/darkages/blob/issue-178/docs/customization.md#triggering-a-cache-update)) (thanks @malberts) 
* Added external link icons support via the new `DarkagesEnableExternalLinkIcons` setting (thanks @malberts)
...
* Added `DarkagesSkinTemplateOutputPageBeforeExec` hook (thanks @jdlrobson)
...
Fixes:
* ([#87](https://github.com/ProfessionalWiki/darkages/issues/87)) Show formedit button on the navbar
* ([#89](https://github.com/ProfessionalWiki/darkages/issues/89)) Show default icon for unknown page tools on the navbar
* ([#91](https://github.com/ProfessionalWiki/darkages/issues/91)) Ensure mediawiki.skinning.content module is loadable
...
* Rename `$datDarkagesExternalLessVariables` to `$datDarkagesExternalStyleVariables`
...
Changes:
* (#49) Allows skins that build on top of Darkages to set a specific layout
...
* Add DarkagesNavbarHorizontalPersonalToolsLinkText hook
* Add DarkagesNavbarHorizontalNewTalkLinkText hook
...
of the Darkages skin for MediaWiki.
