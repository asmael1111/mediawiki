...
* Added theme support via the new `BraveThemeFile` setting (thanks @malberts)
* Added grid breakpoint override support via the new `$cmln-grid-breakpoints` SCSS variable (thanks @malberts)
* Improved MultimediaViewer extension support (requires [cache update](https://github.com/malberts/brave/blob/issue-178/docs/customization.md#triggering-a-cache-update)) (thanks @malberts) 
* Added external link icons support via the new `BraveEnableExternalLinkIcons` setting (thanks @malberts)
...
* Added `BraveSkinTemplateOutputPageBeforeExec` hook (thanks @jdlrobson)
...
Fixes:
* ([#87](https://github.com/ProfessionalWiki/brave/issues/87)) Show formedit button on the navbar
* ([#89](https://github.com/ProfessionalWiki/brave/issues/89)) Show default icon for unknown page tools on the navbar
* ([#91](https://github.com/ProfessionalWiki/brave/issues/91)) Ensure mediawiki.skinning.content module is loadable
...
Changes:
* (#49) Allows skins that build on top of Brave to set a specific layout
...
* Add BraveNavbarHorizontalPersonalToolsLinkText hook
* Add BraveNavbarHorizontalNewTalkLinkText hook
...
of the Brave skin for MediaWiki.
