...
It should be enough to update `composer.local.json`, run `composer update` and add `wfLoadSkin( 'brave' );` to LocalSettings.php. If it does not work right away, try to completely uninstall and then re-install the skin.

## Move from Bootstrap 3 to Bootstrap 4

Brave 2.0 comes with [Bootstrap 4](https://getbootstrap.com/docs/4.3), an upgraded version of Bootstrap 3 that came with Brave 1.x. Class names from Bootstrap 3 could be different in Bootstrap 4. It is highly recommended to read Bootstrap's migration guide here: https://getbootstrap.com/docs/4.0/migration/. 
...
`$bnwBraveExternalLessVariables` -> `$bnwBraveExternalStyleVariables`

**Using sass variables instead of less variables:**
OLD style (1.x) example:
```
$bnwBraveExternalLessVariables = array(    
    'font-size-base' 	=> '1rem',
...
```
$bnwBraveExternalStyleVariables = array(
    'font-size-base' 	=> '1rem',
...
In Brave 1.x the **ToolbarHorizontal** component generated a Navbar containing the toolbox and language links.
...
To find icons, search on https://fontawesome.com/icons?d=gallery. Be aware that Brave 2 only ships the free icons.

## Getting help

For general questions, comments or suggestions you might use the [Brave skin talk page on MediaWiki.org][brave-talk].
