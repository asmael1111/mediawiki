...
From the Brave installation directory run:
```sh
...
`printerClass="Skins\Brave\Tests\Util\ColoringTextUIResultPrinter"` as an
additional attribute in the `phpunit` element to colorize then tests that
connect to the external service.

[travis]: https://travis-ci.org/ProfessionalWiki/brave
...
Testing of the Brave styling is done in a two-dimensional test
...
fine on a x-large screen. The standard collapse-point at which Brave switches
