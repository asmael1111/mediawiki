...
class PersonalTools extends Component {
	...
	protected function getNewtalkNotifier() {
		...
		$newMessagesAlert = $this->getSkinTemplate()->getMsg( 'brave-newmessages' )->text();
	...
	protected function getDropdownToggle(): string {
		...
		if ( $user->isLoggedIn() ) {
			...
			$toolsLinkText = $this->getSkinTemplate()->getMsg( 'brave-loggedin' )->
				params( $user->getName() )->text();

		} else {

			$toolsClass = 'navbar-usernotloggedin';
			$toolsLinkText = $this->getSkinTemplate()->getMsg( 'brave-notloggedin' )->text();

		}

		Hooks::run( 'BraveNavbarHorizontalPersonalToolsLinkText', [ &$toolsLinkText,
			$this->getSkin() ] );
